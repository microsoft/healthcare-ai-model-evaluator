...
public class DataFileService : IDataFileService
{
    ...
    /// <summary>
    /// Estimates tokens for images using OpenAI GPT-4o pricing (high detail mode).
    /// Uses heuristic based on data URL to estimate image dimensions.
    /// </summary>
    /// <param name="dataUrl">Base64 data URL of the image</param>
    /// <returns>Estimated token count</returns>
    public int EstimateImageTokens(string dataUrl)
    {
        ...
        try
        {
            ...
            // Estimate dimensions based on file size (heuristic)
            // Typical JPEG/PNG compression ratios for medical images
            var estimatedPixels = imageBytes.Length * 8; // Rough estimate
            var dimension = (int)Math.Sqrt(estimatedPixels);
            
            // Clamp to reasonable bounds for medical images
            dimension = Math.Max(512, Math.Min(dimension, 4096));
            
            // OpenAI GPT-4o high detail calculation
            // Scale so short side = 768, then compute 512×512 tiles
            var scale = 768.0 / dimension; // Assuming square image for simplicity
            var scaledDimension = (int)Math.Ceiling(dimension * scale);
            
            var tilesX = (int)Math.Ceiling(scaledDimension / 512.0);
            var tilesY = (int)Math.Ceiling(scaledDimension / 512.0);
            var totalTiles = Math.Max(1, tilesX * tilesY);
            
            var tokens = 85 + 170 * totalTiles;
            
            _logger.LogDebug("Estimated image tokens: {Tokens} for {FileSize}KB image (estimated {Dimension}x{Dimension})", 
                tokens, imageBytes.Length / 1024, dimension, dimension);
            
            return tokens;
        }
        catch (Exception ex)
        {
            _logger.LogWarning(ex, "Failed to estimate image tokens, using fallback");
            // Fallback: assume 1024x1024 image
            // 1024x1024 -> scale to 768x768 -> 2x2 tiles = 4 tiles
            return 85 + 170 * 4; // 765 tokens
        }
    }
                    ...
                    foreach (var arrayItem in arrayElement.EnumerateArray())
                    {
                        ...
                        if (inputMapping.Type == "imageurl" && value.StartsWith("data:image"))
                        {
                            try
                            {
                                ...
                                dataContent.TotalTokens = EstimateImageTokens(value); // Images don't count as tokens
            ...
            else
            {
                ...
                if (inputMapping.Type == "imageurl" && value.StartsWith("data:image"))
                {
                    try
                    {
                        var image = await _imageService.ProcessDataUrlAndSaveImageAsync(value);
                        dataContent.Content = image.Id;
                        dataContent.TotalTokens = EstimateImageTokens(value);
                        objectTotalInputTokens += dataContent.TotalTokens;
                        objectTotalTokens += dataContent.TotalTokens;
                    }
                    catch (Exception ex)
